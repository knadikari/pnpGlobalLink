...
class ControlDeamon:
            ...
            if(msgtype=="link"):
                ...
                if(gofornode):#ToDO check the status and return I cant message
                    ...
                    if(isavilable):  #this node is avilable
                        cprint('in av', 'green')
                        #handle=MessageHandle()
                        #####the funct of threading
                        nextList = handle.getNextNodes(jsonmsg)
                        threads = []

                        for i in range(0,len(nextList)):
                            t = threading.Thread(target=self.LinkNext, args=(handle, jsonmsg, reply,nextList[i],))
                            threads.append(t)
                            t.start()
                        for i in threads:
                            i.join()
    ...
    def LinkNext(self,handle,jsonmsg,reply,nextList):
